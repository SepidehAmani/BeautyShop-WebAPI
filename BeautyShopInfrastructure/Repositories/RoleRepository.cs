...
public class RoleRepository : IRoleRepository
{
    ...
    {
        return await _context.Set<UserSelectedRole>().Where(p => p.UserId == userId && !p.IsDelete)
    ...
    {
        return await _context.Set<Role>().Where(p=> !p.IsDelete).ToListAsync(cancellation);
    }

    public async Task<bool> AddRoleToUser(int userId, ICollection<string>? roleNames,CancellationToken cancellation)
    {
        var userExists = await _context.Set<User>().AnyAsync(p => p.Id == userId && !p.IsDelete);
        if (!userExists) return false;

        var existingRoles = await _context.Set<UserSelectedRole>().Where(p => p.UserId == userId && !p.IsDelete).ToListAsync(cancellation);
        ...
        foreach (var roleName in roleNames)
        {
            ...
            if (role != null)
            {
                ...
                _context.Set<UserSelectedRole>().Add(userSelectedRole);
            }
            else
            { return false; }
        }
    ...
    {
        var records = await _context.Set<UserSelectedRole>().Where(p=> p.UserId==userId).ToListAsync(cancellation);
    ...
    {
        return await _context.Set<Role>().Where(p => !p.IsDelete)
    ...
    {
        return await _context.Set<Role>().Where(p => p.Id == roleId && !p.IsDelete)
            .Select(p => new RoleDTO() { Id = p.Id, CreateDate = p.CreateDate, UniqueName = p.UniqueName })
    ...
    {
        return await _context.Set<Role>().AnyAsync(p=> p.UniqueName.ToLower() == name.ToLower() && !p.IsDelete , cancellation);
    }
    ...
    {
        return await _context.Set<Role>().AnyAsync(p => p.Id != roleId && p.UniqueName.ToLower() == name.ToLower() && !p.IsDelete, cancellation);
    }

    public async Task<Role?> GetRoleById(int roleId,CancellationToken cancellation)
    {
        return await _context.Set<Role>().FirstOrDefaultAsync(p => p.Id == roleId && !p.IsDelete);
    }
}
