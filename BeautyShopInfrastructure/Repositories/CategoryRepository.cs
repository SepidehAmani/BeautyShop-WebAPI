...
public class CategoryRepository : ICategoryRepository
{
    ...
    {
        return await _context.Set<Category>().FirstOrDefaultAsync(p => p.Id == id && !p.IsDelete);
    }

    public async Task<ICollection<Category>?> GetChildCategoriesByParentId(int parentId,CancellationToken cancellation)
    {
        return await _context.Set<Category>().Where(p=> p.ParentId==parentId && !p.IsDelete).ToListAsync(cancellation);
    }

    public async Task<ICollection<ParentCatgeoryDTO>?> GetListOfCategoryDTOs(CancellationToken cancellation)
    {
        var parentCategories = await _context.Set<Category>().Where(p => !p.IsDelete && p.ParentId == null)
        ...
        if (parentCategories!=null && parentCategories.Any())
        {
            ...
            {
                item.ChildCategories = await _context.Set<Category>().Where(p => p.ParentId == item.Id && !p.IsDelete)
